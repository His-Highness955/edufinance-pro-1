...
# --- DEBT LEDGER ---
    ...
    # Updated SQL Query to include Last Payment Date
    query = """
    SELECT 
        s.name as 'Student Name', 
        s.class as 'Class', 
        s.total_fees as 'Total Fee', 
        SUM(IFNULL(p.amount, 0)) as 'Total Paid',
        MAX(p.date) as 'Last Payment Date'
    FROM students s
    ...
    cols = ['Student Name', 'Class', 'Total Fee', 'Total Paid', 'Balance Owed', 'Last Payment Date']
    df_debt = df_debt[cols]

    if st.checkbox("Show Only Debtors", value=True):
        df_debt = df_debt[df_debt['Balance Owed'] > 0]
    
    # Handle empty dates for clear reading
    df_debt['Last Payment Date'] = df_debt['Last Payment Date'].fillna("No Payments Recorded")
